...
/* timer_container_of == 1 means driver is in LPS */
void linked_status_chk(struct adapter *adapt, u8 timer_container_of)
{
	u32	i;
	struct sta_info		*psta;
	struct mlme_ext_priv	*pmlmeext = &adapt->mlmeextpriv;
	...
	if (is_client_associated_to_ap(adapt)) {
		...
		rx_chk_limit = 4;
#ifdef CONFIG_ARP_KEEP_ALIVE
		if (!timer_container_of && pmlmepriv->bGetGateway == 1 && pmlmepriv->GetGatewayTryCnt < 3) {
		...
		if (psta) {
			...
			rtw_delba_check(adapt, psta, timer_container_of);
#endif
...
bypass_active_keep_alive:
				...
				if (rx_chk != _SUCCESS) {
					if (pmlmeext->retry == 0) {
						issue_probereq_ex(adapt, &pmlmeinfo->network.Ssid, pmlmeinfo->network.MacAddress, 0, 0, 0, (timer_container_of ? 0 : 1));
						issue_probereq_ex(adapt, &pmlmeinfo->network.Ssid, pmlmeinfo->network.MacAddress, 0, 0, 0, (timer_container_of ? 0 : 1));
						issue_probereq_ex(adapt, &pmlmeinfo->network.Ssid, pmlmeinfo->network.MacAddress, 0, 0, 0, (timer_container_of ? 0 : 1));
					}
				}
		...
		while (!rtw_end_of_queue_search(&dlist, plist)) {
			...
			report_del_sta_event(adapt, psta->cmn.mac_addr, WLAN_REASON_EXPIRATION_CHK, timer_container_of ? true : false, false);
		}
	}
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 15, 0)
	struct adapter *adapt = timer_container_of(adapt, t, mlmeextpriv.survey_timer);
#else
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 15, 0)
	struct adapter *adapt = timer_container_of(adapt, t, mlmeextpriv.link_timer);
#else
	struct adapter *adapt = (struct adapter *)FunctionContext;
#endif
