...
#else
	struct adapter *adapter = timer_container_of(adapter, t, cfg80211_wdinfo.remain_on_ch_timer);
#endif
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 15, 0)
	struct adapter *adapter = timer_container_of(adapter, t, wdinfo.ap_p2p_switch_timer);
#else
	struct adapter *adapter = (struct adapter *)FunctionContext;
#endif
	struct	wifidirect_info		*pwdinfo = &adapter->wdinfo;
